function interface(){
    ...
    const formText = $('.song-on-phone')
    const mainTextSong = $('.song-on-phone-main')
    const closeTextSong = $('.btn-song-on-phone-main-close')
    const seenSongtext = $('.seen-text-song')
    ...
    const changeIterfaces = document.querySelectorAll('.header-bgcolor-icon')
    ...
    changeIterfaces.forEach(changeIterface => {
    ...
    formText.onclick = e => {
        if(e.target.closest('.song-on-phone-main')) {
            if(e.target === closeTextSong)
